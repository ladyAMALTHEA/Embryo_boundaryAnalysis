...
def show_all_traces(all_traces, genotypes=None, genes=None, color_dict=None, individuals=False, zeroed=False, xlim=[5,95], normalize=False, ylim=None, normalize_each=False):
    ...
    for ax, gene in zip(axs, genes):
        for genotype in genotypes:
            ...
            if normalize_each:
                mean_traces -= mean_traces[:, xlim[0]:xlim[1]].min(1)[...,None]
                mean_traces /= mean_traces[:, xlim[0]:xlim[1]].max(1)[...,None]
            if zeroed:
...
def show_genotype_traces(all_traces, genotype='wt', genes=None, individuals=False, zeroed=False, xlim=[10,90], normalize=False, normalize_each=False):
    if genes is None:
        genes = list(all_traces[genotype].keys())
